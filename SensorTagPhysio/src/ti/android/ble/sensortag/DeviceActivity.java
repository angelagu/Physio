...
public class DeviceActivity extends ViewPagerActivity {
  ...
  private static String TAG = "DeviceActivity";
	...
	private StartScreenView mStartScreenView = null;
  ...
  private void enableSensors(boolean enable) {
  	for (Sensor sensor : mEnabledSensors) {
  		...
  		System.out.println(servUuid);
  		System.out.println(mBtGatt.getService(servUuid));
	...
	private void onCharacteristicChanged(String uuidStr, byte[] value) {
		if (mDeviceView != null) {
			...
			if (uuidStr.equals(SensorTag.UUID_GYR_DATA.toString())) {	
				if (((RangeOfMotion) this.getApplication()).isRecording()) {
					((RangeOfMotion) this.getApplication()).appendGyroscopeReading(Sensor.GYROSCOPE.convert(value));
				}
			}
